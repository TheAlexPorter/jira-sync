...
Before proceeding, verify the workflow plugin is available by checking if the `workflow:plan` skill exists.

Use the Glob tool to check for the workflow plugin:
- Glob for `~/.claude/plugins/cache/lendio/workflow/*/commands/plan.md`
...
Once installed, run /jira:plan again.
...
- `cloudId`: the cloudId from step 1.5
